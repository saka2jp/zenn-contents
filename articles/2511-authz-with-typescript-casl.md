...
- **コスト:** 認可判定の回数（Check API のコール数）に応じて課金される SaaS モデルを避け、スケーリングしてもコストが増加しないようにしたい。
- **開発体験:** 外部依存を減らし、`npm install` だけで開発環境が整うシンプルさ。
- **DB 統合:** アプリケーション DB (Prisma) と密結合させ、効率的なクエリを発行したい。
...
export async function getCandidates(user: UserContext) {
  // Ability から Prisma の where 句を生成
  // ユーザーの権限に基づいて、読めるレコードだけの条件式が自動生成される
  const candidates = await prisma.candidate.findMany({
    where: {
      AND: [
        accessibleBy(ability, 'read').Candidate, // ここがポイント！
      ]
    }
  });
...
export function CandidateCard({ candidate }) {
  const ability = useAppAbility() // AppAbilityを生成するカスタムフック
...
3.  **フロントエンドの体験向上**
    「画面を隠す」「ボタンを非活性にする」などの実装が宣言的・直感的にできる点です。フロントエンドエンジニアは認可基盤の具体まで詳細に理解できていなくても、認可ロジックを適切に適用できる基盤を整えることができました。
...
BtoB SaaS の認可は複雑になりがちですが、CASL と Prisma を組み合わせることで、型安全かつ堅牢で疎結合な認可基盤を構築できました。
外部の認可 SaaS を使う選択肢もありますが、アーキテクチャによっては CASL のようなインプロセスなライブラリが強力な選択肢になると感じています。
